...
namespace ThunderPropagator.Channels.Chat.Pipelines.Groups.AddUser
{
    ...
    {
        private const string TelemetryActivityName = "thunderpropagator.channels.chat.groups.adduser";
        private static readonly Counter<long>? TelemetryRequestCounter =
            Telemetry.CreateCounter<long>(TelemetryActivityName, "{request}", "Total add-user-to-group requests received.");
            ...
            CancellationToken cancellationToken)
        {
            ...
            var group = await groupService.AddUserToGroupAsync(currentUserId, addUserRequest.GroupId, user.Id, cancellationToken);
