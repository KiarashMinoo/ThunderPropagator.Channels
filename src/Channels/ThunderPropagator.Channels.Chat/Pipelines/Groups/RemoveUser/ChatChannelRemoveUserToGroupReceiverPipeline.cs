        ...
        : AuthenticatedChatChannelReceiverPipeline(loggerFactory)
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            chatChannel.EmitMessage(new ChatChannelFeederMessage(
                await messageService.SendMessageAsync(currentUserId, user.Id, $"{sender.Name} has removed you from group {group.Name}.", cancellationToken)
            ));

            //Send Add Message To Group
            var messages = await messageService.SendMessageToGroupAsync(currentUserId, group.Id, $"User {sender.Name} has removed user {user.Name} from group.", cancellationToken);
