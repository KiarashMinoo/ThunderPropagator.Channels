namespace ThunderPropagator.Channels.Demo.VideoPlayer.Media
{
    ...
    /// <see cref="AudioFrameEncoder"/> implements for real Opus/AAC encoding, and a deterministic test
    /// double can implement instead so a session's own audio wiring is testable without any native
    /// FFmpeg dependency, mirroring how <c>encodeFrame</c> is already injectable for the video side.
    /// </summary>
    public interface IAudioEncoder : IDisposable
    {
        /// <summary>Which codec this instance encodes to — see <see cref="AudioFrameEncoder.Encoding"/>'s own remarks.</summary>
        AudioFramePacketEncoding Encoding { get; }
